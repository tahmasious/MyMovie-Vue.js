<template>
    <router-link :to="{name : 'person', params : {id : person.id}}" class="thumbnail-image relative mb-3">
        <img
        :src="imageFinalPath"
        alt="movie poster"
        ...
        >
        <div>
            <h3 class="text-white ml-1 text-center">{{person.name}}</h3>
        </div>
        <div>
            <div class="text-center">{{person.popularity}}</div>
...
const imageFinalPath = computed(() => {
    if (props.person.profile_path) {
        return `${API_IMAGE_BASE_URL}/w154${props.person.profile_path}`
    }
    return defaultImage;
})
</script>
